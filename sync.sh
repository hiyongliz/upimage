...
# Usage: ./sync.sh [image_list_file] [region]
...
sync_images() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if $UPIMAGE_BINARY "$image" --region "$REGION"; then
...
# Show usage
show_usage() {
    echo "Usage: $0 [image_list_file] [region]"
    echo
    echo "Arguments:"
    ...
    echo
    echo "Examples:"
    echo "  $0                           # Use default image.txt and cn-south-1"
